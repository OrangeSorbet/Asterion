...
const accretionDiskShader = {
  uniforms: { uTime: { value: 0 }, uInner: { value: 34.0 }, uOuter: { value: 160.0 } },
  vertexShader: `
    varying vec2 vPos;
    void main() {
      vPos = position.xy;
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      float flow = angle * 3.0 - uTime * (2.0 - t) * 1.5;
      float turb = noise(vec2(flow * 2.0, t * 6.0)) * 0.6 + noise(vec2(flow * 5.0, t * 10.0)) * 0.4;
      float edgeFade = smoothstep(0.0, 0.12, t) * smoothstep(1.0, 0.7, t);
      float heat = 1.0 - t;
...
function buildBlackHole() {
  ...
  const accGeo = new THREE.RingGeometry(34, 160, 256, 1);
